...
const president = world.startingTerms.find((term) => world.offices[term.officeId]?.kind === "president")!.holderId;
...
for (let index = 0; index < seeds; index++) {
  ...
  const startProvinceRange = range(Object.values(state.economyRuntime.provinces).map((row) => row.conditionsIndex));
  const startSectorRange = range(Object.values(state.economyRuntime.sectors).map((row) => row.conditionsIndex));
  let yearOne = start;
  ...
  for (let month = 1; month <= months; month++) {
    ...
    const provinceSpread = range(Object.values(state.economyRuntime.provinces).map((row) => row.conditionsIndex)).spread;
    ...
    const visible = [
      ...
      ...Object.values(state.economyRuntime.provinces).flatMap((row) => [row.conditionsIndex, row.employmentIndex, row.housingIndex]),
  ...
  const signChanges = outputChanges.slice(1).filter((value, at) => Math.sign(value) !== Math.sign(outputChanges[at]!)).length;
  runs.push({
    seed,
    start,
    startProvinceRange,
    ...
    finalProvinceRange: range(Object.values(state.economyRuntime.provinces).map((row) => row.conditionsIndex)),
    maxProvinceSpread,
    shocks: state.history.filter((event) => event.type === "ECONOMIC_SHOCK").length,
    boundHits,
    averageMonthlyOutputMove: outputChanges.reduce((sum, value) => sum + Math.abs(value), 0) / outputChanges.length,
...
console.log(JSON.stringify({
  economyCalibration: {
    ...
    averageMonthlyOutputMove: runs.reduce((sum, run) => sum + run.averageMonthlyOutputMove, 0) / runs.length,
    averageDirectionChanges: runs.reduce((sum, run) => sum + run.outputDirectionChanges, 0) / runs.length,
  },
  runs,
}, null, 2));
