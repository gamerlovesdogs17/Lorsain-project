...
const national = campaignVisit(
  world,
  state,
  rng,
  { campaignId: campaign.id, actorId: politicianId, geography: { kind: "national", id: null } },
  "CAL_NATIONAL",
);
if ("error" in national) throw new Error(national.error.message);
const sortedIds = Object.keys(world.constituencyElectorate).sort();
const firstFourAverage =
  sortedIds
    .slice(0, 4)
    .reduce((sum, id) => sum + (campaign.organizationByConstituency[id] ?? 0), 0) / 4;
const laterAverage =
  sortedIds.slice(4).reduce((sum, id) => sum + (campaign.organizationByConstituency[id] ?? 0), 0) /
  Math.max(1, sortedIds.length - 4);
...
const province = campaignOrganize(
  world,
  state,
  {
    campaignId: campaign.id,
    actorId: politicianId,
    geography: { kind: "province", id: provinceId },
  },
  "CAL_PROVINCE",
);
...
const constituency = campaignOrganize(
  world,
  state,
  {
    campaignId: campaign.id,
...
console.log(
  JSON.stringify(
    {
      campaignGeographyCalibration: {
        nationalConstituenciesReached: sortedIds.filter(
          (id) => (campaign.organizationByConstituency[id] ?? 0) > 0,
        ).length,
        nationalProvincesReached: world.provinceIds.filter((id) => (beforeProvince[id] ?? 0) > 0)
          .length,
        firstFourAverageAfterNational: firstFourAverage,
        laterAverageAfterNational: laterAverage,
        nationalActionOnlyFirstFour: sortedIds
          .slice(4)
          .every((id) => (campaign.organizationByConstituency[id] ?? 0) === 0),
        provinceAction: {
          provinceId,
          gain:
            (campaign.organizationByProvince[provinceId] ?? 0) - (beforeProvince[provinceId] ?? 0),
        },
        constituencyAction: { constituencyId, gain: beforeDecay - beforeConstituency },
        twelveMonthPersistence: {
          before: beforeDecay,
          after: campaign.organizationByConstituency[constituencyId] ?? 0,
        },
