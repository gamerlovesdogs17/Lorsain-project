...
const leadershipOutputPath = resolve(
  repoRoot,
  "docs/qa/phase11_3/leadership-election-browser-save.json",
);
const formerOfficeholderOutputPath = resolve(
  repoRoot,
  "docs/qa/phase11_3/former-officeholder-browser-save.json",
);
const electionResultsOutputPath = resolve(
  repoRoot,
  "docs/qa/phase11_3/election-results-browser-save.json",
);
...
const provincialMemberIds = new Set(
  Object.values(leadershipSave.simulation.provincialRuntime.assemblies).flatMap(
    (assembly) => assembly.memberIds,
  ),
);
const provincialLegislator = Object.values(leadershipSave.simulation.provincialRuntime.legislators)
  .filter(
    (row) =>
      provincialMemberIds.has(row.id) &&
      row.active &&
      row.serviceEndDate == null &&
      row.fullPoliticianId == null,
  )
  .sort((a, b) => a.id.localeCompare(b.id))[0];
if (!provincialLegislator)
  throw new Error("No Provincial Assembly member was available for browser QA promotion.");
...
if (!provincialPlayerId)
  throw new Error("The Provincial Assembly browser-QA politician could not be promoted.");
...
if (stillInOffice)
  throw new Error(
    "The former-officeholder browser fixture did not complete the presidential transition.",
  );
writeFileSync(
  formerOfficeholderOutputPath,
  `${JSON.stringify(formerOfficeholderSave, null, 2)}\n`,
  "utf8",
);
...
const governorResolved = Object.values(
  electionResultsSave.simulation.provincialRuntime.elections,
).some((election) => election.status === "resolved" || election.status === "assumed");
const provincialAssemblyResolved = Object.values(
  electionResultsSave.simulation.provincialRuntime.assemblyElections,
).some((election) => election.status === "resolved");
if (!federalAssemblyResolved || !governorResolved || !provincialAssemblyResolved) {
  throw new Error(
    `The election-results browser fixture did not certify every required election family (${electionResultsSave.simulation.currentDate}; federal=${federalAssemblyResolved}; governor=${governorResolved}; provincial=${provincialAssemblyResolved}).`,
  );
}
writeFileSync(
  electionResultsOutputPath,
  `${JSON.stringify(electionResultsSave, null, 2)}\n`,
  "utf8",
);
...
console.log(
  `${formerOfficeholderOutputPath} (${provincialPlayerId} is the Provincial Assembly QA player)`,
);
