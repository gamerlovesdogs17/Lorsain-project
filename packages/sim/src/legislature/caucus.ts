...
function leadershipScore(
  ...
  return role === "floor_leader"
    ? profile.skills.legislation * 0.34 +
        profile.skills.negotiation * 0.25 +
        profile.skills.media * 0.12 +
        profile.traits.ambition * 0.17 +
        profile.traits.partyLoyalty * 0.12
    : profile.skills.legislation * 0.22 +
        profile.skills.negotiation * 0.27 +
        profile.traits.partyLoyalty * 0.3 +
        profile.traits.pragmatism * 0.21;
}
...
function selectInitialOfficer(
  ...
  return (
    assemblyCaucus(world, state, partyId)
      .filter((id) => id !== state.playerPoliticianId && id !== exclude)
      .sort(
        (a, b) =>
          leadershipScore(world, state, b, role) - leadershipScore(world, state, a, role) ||
          a.localeCompare(b),
      )[0] ?? null
  );
}
...
export function seedCaucusLeadership(world: KernelWorld, state: SimState): void {
  const represented = new Set(
    Object.values(state.officeTerms)
      .filter(
        (term) =>
          term.status === "active" && world.offices[term.officeId]?.kind === "assembly_member",
      )
  ...
  for (const candidateId of contest.candidateIds) {
    ...
    contest.endorsements[candidateId] = caucus
      ...
      .sort((a, b) => {
        ...
        const sameA =
          candidateFaction && state.politicians[a]?.factionId === candidateFaction ? 1 : 0;
        const sameB =
          candidateFaction && state.politicians[b]?.factionId === candidateFaction ? 1 : 0;
        return (
          sameB - sameA ||
          leadershipScore(world, state, b, contest.role) -
            leadershipScore(world, state, a, contest.role) ||
          a.localeCompare(b)
        );
      })
      .slice(0, 2);
...
function resolveContest(
  ...
  const totals: Record<string, number> = Object.fromEntries(
    contest.candidateIds.map((id) => [id, 0]),
  );
  for (const electorId of electors) {
    const ranked = contest.candidateIds.slice().sort((a, b) => {
      const score = (candidateId: string): number => {
        ...
        const sameFaction =
          state.politicians[electorId]?.factionId &&
          state.politicians[electorId]?.factionId === state.politicians[candidateId]?.factionId
            ? 0.08
            : 0;
        const endorsement = contest.endorsements[candidateId]?.includes(electorId) ? 0.14 : 0;
        const platform = contest.platforms[candidateId];
        const platformFit =
          platform === "party_unity"
            ...
            : platform === "electoral_recovery"
              ...
              : ((profile?.skills.legislation ?? 0.5) + (profile?.skills.negotiation ?? 0.5)) *
                0.05;
        const noise =
          (stableHash(`${contest.id}:${electorId}:${candidateId}:noise`) % 1001) / 40000;
        return (
          leadershipScore(world, state, candidateId, contest.role) +
          sameFaction +
          endorsement +
          platformFit +
          noise
  ...
  contest.winnerId =
    contest.candidateIds
      .slice()
      .sort((a, b) => (totals[b] ?? 0) - (totals[a] ?? 0) || a.localeCompare(b))[0] ?? null;
...
export function processCaucusLeadershipMonth(
  world: KernelWorld,
  state: SimState,
  ...
  for (const leadership of Object.values(state.legislatureRuntime.caucusLeadership).sort((a, b) =>
    ...
    const afterGeneral =
      latestGeneral != null &&
      (!leadership.selectedDate || latestGeneral.date > leadership.selectedDate);
    const vacancy =
      !leadership.floorLeaderId ||
      !leadership.whipId ||
      !caucus.has(leadership.floorLeaderId) ||
      !caucus.has(leadership.whipId);
    const cohesion = state.partyStates[leadership.partyId]?.cohesion ?? 0.65;
    const challengeEligible =
      leadership.selectedDate != null &&
      state.currentDate >= addMonths(leadership.selectedDate, 18) &&
      cohesion < 0.5 &&
    ...
    const reviewTriggered =
      reviewDue &&
      (cohesion < 0.62 ||
        stableHash(`${leadership.partyId}:${state.currentDate.slice(0, 4)}:review`) % 100 < 20);
    ...
    const roles =
      trigger === "vacancy"
        ? [
            ...(!leadership.floorLeaderId || !caucus.has(leadership.floorLeaderId)
              ? ["floor_leader" as const]
              : []),
            ...(!leadership.whipId || !caucus.has(leadership.whipId) ? ["whip" as const] : []),
          ]
        : (["floor_leader", "whip"] as const);
    for (const role of roles) {
      const open = Object.values(state.legislatureRuntime.caucusContests).some(
        (contest) =>
          contest.partyId === leadership.partyId &&
          contest.role === role &&
          contest.status === "open",
      );
      if (!open) {
        ...
        if (contest)
          events.push(
            pushHistory(state, {
              ...
              type: "CAUCUS_LEADERSHIP_ELECTION_OPENED",
              importance: 0.46,
              visibility: "public",
              actorIds: [],
              entityIds: [contest.id, contest.partyId],
              payload: { contestId: contest.id, partyId: contest.partyId, role, trigger },
  ...
  for (const contest of Object.values(state.legislatureRuntime.caucusContests).sort((a, b) =>
    ...
    events.push(
      pushHistory(state, {
        date: state.currentDate,
        type: "CAUCUS_LEADERSHIP_ELECTION_RESOLVED",
        importance: 0.58,
        visibility: "public",
...
export function declareCaucusLeadershipCandidacy(
  ...
  if (actorId !== state.playerPoliticianId)
    return {
      error: reject("PLAYER_AUTONOMY", "The player may only enter their own caucus contest"),
    };
  const contest = state.legislatureRuntime.caucusContests[contestIdValue];
  if (!contest || contest.status !== "open")
    return { error: reject("INVALID_CONTEST", contestIdValue) };
  if (!assemblyCaucus(world, state, contest.partyId).includes(actorId))
    return { error: reject("NOT_CAUCUS_MEMBER", actorId) };
  if (contest.playerDecision != null)
    return { error: reject("DECISION_ALREADY_MADE", contestIdValue) };
  ...
  return {
    events: [
      pushHistory(state, {
        date: state.currentDate,
        type: "CAUCUS_LEADERSHIP_CANDIDACY_DECLARED",
        importance: 0.48,
        visibility: "public",
        actorIds: [actorId],
        entityIds: [contest.id, contest.partyId],
        payload: { contestId: contest.id, partyId: contest.partyId, role: contest.role },
        sourceScheduledEventId: null,
        sourceCommandId: commandId,
      }),
    ],
...
export function campaignCaucusLeadership(
  ...
  if (actorId !== state.playerPoliticianId)
    return {
      error: reject(
        "PLAYER_AUTONOMY",
        "The player may only campaign for their own caucus candidacy",
      ),
    };
  const contest = state.legislatureRuntime.caucusContests[contestIdValue];
  if (!contest || contest.status !== "open")
  ...
  contest.endorsements[actorId] = caucus
    ...
    .sort((a, b) => {
      const sameA =
        state.politicians[a]?.factionId &&
        state.politicians[a]?.factionId === state.politicians[actorId]?.factionId
          ? 1
          : 0;
      const sameB =
        state.politicians[b]?.factionId &&
        state.politicians[b]?.factionId === state.politicians[actorId]?.factionId
          ? 1
          : 0;
      return (
        sameB - sameA ||
  ...
  return {
    events: [
      pushHistory(state, {
        date: state.currentDate,
        type: "CAUCUS_LEADERSHIP_CAMPAIGN",
        importance: 0.38,
        visibility: "public",
        actorIds: [actorId, ...contest.endorsements[actorId]],
        entityIds: [contest.id, contest.partyId],
        payload: {
          contestId: contest.id,
          partyId: contest.partyId,
          role: contest.role,
          emphasis,
          endorsements: contest.endorsements[actorId],
        },
        sourceScheduledEventId: null,
        sourceCommandId: commandId,
      }),
