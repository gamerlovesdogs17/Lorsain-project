...
function Party(props) {
    ...
    return (_jsxs("div", { children: [_jsx(PageHeader, { kicker: "Parties and caucuses", title: party?.name ?? "No party", subtitle: "National party directory, internal elections, caucuses, and parliamentary leadership." }), _jsx("div", { className: "party-directory-strip", role: "navigation", "aria-label": "All parties", children: availablePartyIds.map((id) => {
                    ...
                    return _jsxs("button", { type: "button", className: `party-directory-item${id === partyId ? " selected" : ""}`, style: { borderLeftColor: partyColor(props.world, id) }, onClick: () => { setSelectedPartyId(id); setSelectedFactionId(null); props.setGlobalFocus({ kind: "Party", id }); }, children: [_jsx("strong", { children: partyDisplayName(props.world, id, props.snap) }), _jsxs("span", { children: [seats, " seats \u00B7 ", leader ? politicianDisplayName(props.catalog, leader) : "leadership vacant"] })] }, id);
...
function Terena(props) {
    ...
    return (_jsxs("div", { children: [_jsx(PageHeader, { kicker: "Geography", title: "Terena", subtitle: "Interactive map derived from canonical GeoJSON. Supplied SVG files remain authoring references." }), _jsx(TabBar, { tabs: [
                    ...
                    { id: "economy", label: "Economy" },
                        ...
                        }, children: electionChoices.map((election) => _jsxs("option", { value: election.id, children: [election.provinceId ? `${props.catalog.places.get(election.provinceId)?.name ?? election.provinceId} · ` : "", election.date.slice(0, 4), " \u00B7 ", election.type.replace(/_/g, " "), " \u00B7 ", election.status.replace(/_/g, " ")] }, election.id)) })] })) : null, mode === "campaign" ? _jsxs("div", { className: "map-scale-switch", "aria-label": "Campaign map scale", children: [_jsx("button", { type: "button", className: campaignMapScale === "province" ? "active" : "", onClick: () => { setCampaignMapScale("province"); setSel(null); }, children: "Provinces" }), _jsx("button", { type: "button", className: campaignMapScale === "constituency" ? "active" : "", onClick: () => { setCampaignMapScale("constituency"); setSel(null); }, children: "Constituencies" })] }) : null, _jsx(MapDetailLayout, { className: "terena-map-workspace", detailVisible: sel != null, map: _jsxs(_Fragment, { children: [_jsx(TerenaMap, { bundle: props.bundle, mode: mode, selectedId: sel?.id ?? null, showConstituencies: mode !== "economy" && !(mode === "election" && activeMapElection?.type !== "assembly") && !(mode === "campaign" && campaignMapScale === "province"), fillFor: (f, kind) => mapFillFor(mode, props.world, props.snap, f, kind, props.campaign?.organizationByConstituency, props.campaign?.organizationByProvince, activeMapElection?.id), onSelect: setSel, onHover: (selection) => {
