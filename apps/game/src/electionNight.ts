 ...
 */
export function assemblyReportingOrder<T extends {
  ...
  const score = (row: T) => {
    ...
    const jitter = (stableElectionNightHash(`${electionId}:${row.constituencyId}:report`) % 1000) / 1000;
    return transferWork * 0.38 + magnitude * 0.2 + close * 0.24 + volume * 0.14 + jitter * 0.04;
  };
  return rows.slice().sort((a, b) => score(a) - score(b) || a.constituencyId.localeCompare(b.constituencyId));
}

export function provinceReportingOrder<T extends {
  id: string;
  provinceId: string;
