...
function pollDatum(poll: PollRecord): PublicGeographicDatum {
  ...
  const ranked = [...byParty.entries()].sort(
...
export function publicForecast(
  ...
  if (direct) {
    ...
    const byParty = new Map<string | null, number>();
    for (const row of direct.firstPreference)
      byParty.set(row.partyId, (byParty.get(row.partyId) ?? 0) + row.share);
    const projectedSeats =
      kind === "constituency"
        ? projectSeats(
            [...byParty.entries()],
            world.constituencyElectorate[geographyId]?.seats ?? 0,
          )
        : undefined;
    return {
      ...datum,
      truth: "forecast",
      label: `${datum.category ?? "Toss-up"} public forecast`,
      detail: `${datum.detail}. Forecast uses only this published local poll${projectedSeats?.length ? " and a largest-remainder STV seat approximation" : ""}; it is not an official result.`,
      confidence: "direct",
  ...
  if (previous.truth !== "no_data") {
    return {
      ...previous,
      truth: "forecast",
      label: previous.leaderPartyId ? "Low-confidence public lean" : "Public forecast: competitive",
      detail: `${previous.detail}. No direct current local poll; this is a low-confidence structural forecast and not an official result.`,
      confidence: "low",
      category: previous.leaderPartyId ? "Lean" : "Toss-up",
    };
  }
  ...
  if (publicDelegation.length) {
    return {
      truth: "forecast",
      leaderPartyId,
      label: leaderPartyId ? "Low-confidence public lean" : "Public forecast: competitive",
      detail:
        "Based on current public Assembly representation; no direct local poll is available. This is not an official result.",
      asOf: state.currentDate,
      confidence: "low",
      category: leaderPartyId ? "Lean" : "Toss-up",
      ...(kind === "constituency" ? { projectedSeats: publicDelegation } : {}),
    };
  }
  return {
    truth: "no_data",
