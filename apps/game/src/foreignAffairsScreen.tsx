...
type ForeignDeskTab =
  "overview" | "relations" | "treaties" | "trade" | "security" | "crises" | "history";
...
export function ForeignAffairsPage(props: {
  ...
  return (
    <div className="foreign-affairs-page" data-qa="foreign-affairs">
      ...
      {deskTab === "treaties" ? (
        <div data-qa="fa-treaties">
          <SectionCard title="Treaties">
            <table className="data-table" data-qa="fa-treaties-table">
              <thead>
                <tr>
                  <th>Title</th>
                  <th>Type</th>
              ...
              <tbody>
                {Object.values(runtime.treaties)
                  ...
                  .map((t) => (
                    <tr key={t.id}>
                      <td>{t.title || t.id}</td>
                      <td>{treatyTypeLabel(t.kind)}</td>
                      <td>{treatyStatusLabel(t)}</td>
                      <td>{t.memberIds.map((id) => countryDisplayName(world, id)).join(", ")}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
      ...
      {deskTab === "trade" ? (
        <div data-qa="fa-trade">
          <SectionCard title="Trade exposure">
            <table className="data-table" data-qa="fa-trade-table">
              <thead>
                <tr>
                  <th>Country</th>
              ...
              <tbody>
                {Object.keys(runtime.countries)
                  .filter((id) => id !== TERENA_WORLD_ID)
                  .sort()
                  .slice(0, 30)
                  .map((id) => {
                    const key = bilateralKey(TERENA_WORLD_ID, id);
                    const rel = runtime.bilateralRelations[key];
                    return (
                      <tr key={id}>
                        <td>{countryDisplayName(world, id)}</td>
                        <td>{formatPublicPercent(runtime.countries[id]?.tradeExposure ?? 0)}</td>
                        <td>{formatPublicPercent(rel?.economicTies ?? 0)}</td>
                      </tr>
                    );
                  })}
              </tbody>
      ...
      {deskTab === "security" ? (
        <div data-qa="fa-security">
          <SectionCard title="Security & posture">
            <p>
              Terena military posture:{" "}
              <strong>{militaryPostureLabel(terenaRuntime?.posture ?? "normal")}</strong>
            </p>
            <table className="data-table">
              ...
              <tbody>
                {Object.entries(runtime.countries)
                  .filter(([, c]) => c.posture && c.posture !== "normal")
                  .sort(([a], [b]) => a.localeCompare(b))
                  .slice(0, 25)
                  .map(([id, c]) => (
                    <tr key={id}>
                      <td>{countryDisplayName(world, id)}</td>
                      <td>{militaryPostureLabel(c.posture)}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
      ...
      {deskTab === "crises" ? (
        <div data-qa="fa-crises">
          <SectionCard title="Crises">
            {publicCrises.length === 0 ? (
              <EmptyState>No public crises are active.</EmptyState>
            ) : (
              publicCrises.map((c) => (
                <ActivityFeedItem
                  key={c.id}
                  date={c.startedDate}
                  text={`${c.participantIds.map((id) => countryDisplayName(world, id)).join("–")} · ${crisisStageLabel(c.stage)} · ${publicSeverityLabel(c.intensity, c.stage)}`}
                />
              ))
            )}
          </SectionCard>
      ...
      {deskTab === "history" ? (
        <div data-qa="fa-history">
          <SectionCard title="Diplomatic history">
            <ol className="wiki-timeline">
              {snap.history
                .filter(
                  (e) =>
                    e.visibility === "public" &&
                    /TREATY|SANCTION|CRISIS|CONFLICT|DIPLOMATIC|FOREIGN|DOMESTIC_FOREIGN/.test(
                      e.type,
                    ),
                )
                .slice(-40)
                ...
                .map((e) => (
                  <li key={e.id}>
                    <time>{e.date}</time>
                    <span>{eventDisplay(catalog, world, snap, e)}</span>
                  </li>
                ))}
            </ol>
          </SectionCard>
          ...
          <div data-qa="fa-map-desk">
            <MetricStrip>
              <StatCard
                ...
                value={president ? `${capacityUsed}/${MAX_DIPLOMATIC_ACTIONS_PER_MONTH}` : "—"}
                hint={
                  president ? `${capacityLeft} remaining this month` : "Presidential prerogative"
                }
              />
              <StatCard
                label="Active crises"
                ...
                <div className="foreign-actions-panel">
                  <SectionCard title="Presidential diplomacy">
                    {capacityLeft <= 0 ? (
                      ...
                      <p className="muted">
                        {capacityLeft} presidential action{capacityLeft === 1 ? "" : "s"} remaining
                        this month.
                      </p>
                    )}
            ...
            <div className="foreign-global-panels">
              ...
              <SectionCard title="Sanctions in force">
                ...
                {activeSanctions.slice(0, 8).map((s) => (
                  <div key={s.id} className="muted">
                    {countryDisplayName(world, s.imposerId)} →{" "}
                    {countryDisplayName(world, s.targetId)} ·{sanctionsScopeLabel(s.severity)}{" "}
                    measures
                  </div>
                ))}
              </SectionCard>
