...
function Executive(props: {
  ...
  const [appointOfficeId, setAppointOfficeId] = useState<string>(
  ...
  const eligible = useMemo(() => {
    ...
    return Object.keys(props.snap.politicians)
      .filter((id) => {
        ...
        if (!q) return true;
        const fig = props.figures.get(id);
        const offices = playerOffices(props.world, props.snap, id).join(" ");
        const party = partyName(props.world, props.snap.politicians[id]?.partyId ?? null);
        const hay = `${fig?.name ?? id} ${party} ${offices} ${fig?.office ?? ""}`.toLowerCase();
        return hay.includes(q);
      })
      .sort((a, b) =>
        politicianName(props.figures, a).localeCompare(politicianName(props.figures, b)),
      )
      .slice(0, 60);
  }, [appointQuery, props.figures, props.snap, props.world, selectedOfficeId]);
  return (
    <div className="card">
      <h3>Executive</h3>
          ...
          >
            {eligible.map((id) => {
              ...
              return (
                ...
                >
                  <div>
                    <strong>{politicianName(props.figures, id)}</strong>
                    <div className="muted">
                      {partyName(props.world, props.snap.politicians[id]?.partyId ?? null)}
                      {offices.length ? ` · ${offices.join(", ")}` : ""}
                      {!offices.length && fig?.office ? ` · ${fig.office}` : ""}
                    </div>
                  </div>
                </button>
              );
