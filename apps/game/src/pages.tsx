          ...
          <>
            ...
            {playerIsMp && constituencyPressures.length ? (
              <div className="home-constituency-brief">
                <SectionDivider title="Constituency brief" hint="Public pressures shaping local politics" />
                {constituencyPressures.slice(0, 4).map((pressure) => (
                  <EntityRow
                    key={pressure.kind}
                    title={pressure.label}
                    meta={pressure.detail}
                    status={<StatusBadge tone={pressure.level === "urgent" ? "warn" : "idle"}>{pressure.level === "urgent" ? "Urgent" : pressure.level === "important" ? "Important" : "Watch"}</StatusBadge>}
                  />
                ))}
              </div>
            ) : null}
            <SectionDivider title="Why this standing?" hint="Public context, not a hidden formula" />
            <p className="muted">
              Your {standingLabel} standing is presented alongside recent public conduct and the conditions attached to your office.
            </p>
            {standingContext.length ? standingContext.map((event) => (
              <ActivityFeedItem key={event.id} date={event.date} text={eventDisplay(props.catalog, props.world, props.snap, event)} />
            )) : (
              <EmptyState>No recent personal event dominates the public record.</EmptyState>
            )}
            <p className="muted">
              Economic context: {governorPublicEconomy?.summary ?? `${publicEconomy.growth.toFixed(1)}% annual output growth and ${publicEconomy.confidenceTrend.toLowerCase()} confidence`}.
            </p>
          </>
        }
      />
...
function Career(props: PageProps) {
  const [tab, setTab] = useState<"opportunities" | "overview" | "career" | "positions" | "record" | "directory" | "comparison" | "figures">(
    "opportunities",
  );
    ...
    <div>
      ...
      {tab === "record" ? (
        <SectionCard title="Public history">
          {props.snap.history.filter((e) => e.visibility === "public" && e.actorIds.includes(props.snap.playerPoliticianId)).length === 0 ? <EmptyState>No public career events are recorded yet.</EmptyState> : null}
          {[...new Set(props.snap.history.filter((e) => e.visibility === "public" && e.actorIds.includes(props.snap.playerPoliticianId)).map((e) => e.date.slice(0, 4)))].sort().reverse().map((year) => <section className="profile-history-year" key={year}>
            <strong>{year}</strong>
            <div>{props.snap.history.filter((e) => e.visibility === "public" && e.actorIds.includes(props.snap.playerPoliticianId) && e.date.startsWith(year)).slice().reverse().map((e) => <ActivityFeedItem key={e.id} date={e.date} text={eventDisplay(props.catalog, props.world, props.snap, e)} />)}</div>
          </section>)}
        </SectionCard>
      ) : null}
      {tab === "directory" ? (() => {
        ...
        return <WorkLayout
          main={<>
            ...
            <div className="directory-count-row"><p className="muted">{rows.length} matching public figures · {PAGE_SIZE} per page</p>{comparisonIds.length >= 2 ? <button type="button" className="btn btn-sm secondary" onClick={() => setTab("comparison")}>Compare selected ({comparisonIds.length})</button> : null}</div>
            <div className="entity-list politician-directory-list">{visibleRows.map((politician) => <div className="politician-directory-row" key={politician.id}>
              <PoliticianCard catalog={props.catalog} world={props.world} state={props.snap} politicianId={politician.id} compact selected={politician.id === selected?.id} onSelect={() => setDirectorySelection(politician.id)} />
              <button type="button" className={`compare-toggle${comparisonIds.includes(politician.id) ? " selected" : ""}`} aria-pressed={comparisonIds.includes(politician.id)} disabled={!comparisonIds.includes(politician.id) && comparisonIds.length >= 3} onClick={() => setComparisonIds((ids) => ids.includes(politician.id) ? ids.filter((id) => id !== politician.id) : [...ids, politician.id].slice(0, 3))}>{comparisonIds.includes(politician.id) ? "Comparing" : "Compare"}</button>
            </div>)}</div>
            {pageCount > 1 ? <div className="pager"><button type="button" className="btn secondary" disabled={page === 0} onClick={() => setDirectoryPage(page - 1)}>Previous</button><span className="muted">Page {page + 1} of {pageCount}</span><button type="button" className="btn secondary" disabled={page >= pageCount - 1} onClick={() => setDirectoryPage(page + 1)}>Next</button></div> : null}
          </>}
          rail={selected ? <>
            ...
            <SectionDivider title="Institutional roles" />
             ...
             {selectedEndorsements.length + selectedOrganizationEndorsements.length === 0 ? <EmptyState>No public endorsement is recorded.</EmptyState> : <>
             {selectedEndorsements.map((endorsement) => {
               ...
               return <EntityRow key={endorsement.id} title={giving ? `Endorsed ${other}` : `Endorsed by ${other}`} meta={`${contestDisplayName(props.snap, props.world, endorsement.contestId)} · ${endorsement.date}`} status={<StatusBadge tone={endorsement.status === "active" ? "ok" : "idle"}>{endorsement.status === "active" ? "Current" : endorsement.status[0]!.toUpperCase() + endorsement.status.slice(1)}</StatusBadge>} />;
             })}
             {selectedOrganizationEndorsements.map(({ organizationId, endorsement }, index) => {
               ...
               return <EntityRow key={`${organizationId}:${endorsement.campaignId ?? "campaign"}:${index}`} title={`Endorsed by ${props.world.interestOrganizations[organizationId]?.name ?? "Public organization"}`} meta={`${campaign ? campaignTypeLabel(campaign.type) : "Political campaign"} · ${endorsement.date}${endorsement.withdrawnDate ? ` · withdrawn ${endorsement.withdrawnDate}` : ""}`} status={<StatusBadge tone={(endorsement.status ?? "active") === "active" ? "ok" : "idle"}>{(endorsement.status ?? "active") === "active" ? "Current" : "Withdrawn"}</StatusBadge>} />;
             })}
             </>}
            ...
            {props.snap.history.filter((event) => event.visibility === "public" && event.actorIds.includes(selected.id)).slice(-8).reverse().map((event) => <ActivityFeedItem key={event.id} date={event.date} text={eventDisplay(props.catalog, props.world, props.snap, event)} />)}
            {props.snap.history.filter((event) => event.visibility === "public" && event.actorIds.includes(selected.id)).length === 0 ? <EmptyState>No public career event is recorded.</EmptyState> : null}
          </> : <EmptyState>Select a politician.</EmptyState>}
        />;
      })() : null}
      {tab === "comparison" ? (() => {
        const selected = comparisonIds.map((id) => props.snap.politicians[id]).filter((row): row is NonNullable<typeof row> => Boolean(row));
        const activeOffice = (id: string) => Object.values(props.snap.officeTerms).filter((term) => term.holderId === id && (term.status === "active" || term.status === "suspended")).map((term) => props.world.offices[term.officeId]?.title).filter(Boolean).join(", ") || "Private citizen";
        ...
        return <div className="politician-comparison"><PageHeader kicker="Public record" title="Compare politicians" subtitle="Public offices, affiliations, standing, elections, leadership, and roll calls. Hidden personality and strategy are not shown." />
          {selected.length < 2 ? <EmptyState>Select two or three politicians from the directory to compare.</EmptyState> : <DataTable headers={["Public fact", ...selected.map((politician) => politicianDisplayName(props.catalog, politician.id))]}><>{rows.map(([label, value]) => <tr key={label}><th scope="row">{label}</th>{selected.map((politician) => <td key={politician.id}>{value(politician.id)}</td>)}</tr>)}</></DataTable>}
          {selected.length ? <div className="button-row">{selected.map((politician) => <button key={politician.id} type="button" className="btn btn-sm secondary" onClick={() => setComparisonIds((ids) => ids.filter((id) => id !== politician.id))}>Remove {politicianDisplayName(props.catalog, politician.id)}</button>)}</div> : null}
        </div>;
      })() : null}
      {tab === "figures" ? (() => {
        const activeKind = (id: string, kind: string) => Object.values(props.snap.officeTerms).some((term) => term.holderId === id && (term.status === "active" || term.status === "suspended") && props.world.offices[term.officeId]?.kind === kind);
        ...
        const ranked = (ids: string[]) => ids.filter((id) => props.snap.politicians[id]?.alive && !props.snap.politicians[id]?.retired).sort((a, b) => publicAchievementScore(b) - publicAchievementScore(a) || politicianDisplayName(props.catalog, a).localeCompare(politicianDisplayName(props.catalog, b))).slice(0, 8);
        ...
        return <div><PageHeader kicker="Political class" title="Figures to watch" subtitle="Contextual public prominence based on office, standing, electoral success, leadership, and legislative work—not hidden potential." /><div className="figures-watch-grid">{groups.map((group) => <SectionCard key={group.title} title={group.title}>{group.ids.length === 0 ? <EmptyState>No public figure currently qualifies.</EmptyState> : group.ids.map((id) => <PoliticianCard key={id} catalog={props.catalog} world={props.world} state={props.snap} politicianId={id} compact descriptor={publicStandingLabel(props.world, props.snap, id)} onSelect={() => { setDirectorySelection(id); setTab("directory"); }} />)}</SectionCard>)}</div></div>;
      })() : null}
    </div>
  );
}
...
function Party(props: PageProps) {
  ...
  return (
    <div>
      <PageHeader kicker="Parties and caucuses" title={party?.name ?? "No party"} subtitle="National party directory, internal elections, caucuses, and parliamentary leadership." />
      ...
      <SectionCard title="Public platform">
        {runtime?.publicPlatform ? <>
          <p className="muted">Public issue positions move gradually with party identity, caucus influence, and leadership. Updated {runtime.publicPlatform.updatedDate}.</p>
          <div className="party-platform-grid">
            {PARTY_PLATFORM_ISSUES.map((issue) => (
              <div key={issue} className="party-platform-position">
                ...
                <strong>{partyPlatformLabel(issue, runtime.publicPlatform!.positions[issue])}</strong>
              </div>
            ))}
          </div>
          <details className="platform-history">
            <summary>Recent published platforms ({runtime.publicPlatform.history.length})</summary>
            {runtime.publicPlatform.history.length === 0 ? <EmptyState>No prior platform publication is recorded in this save.</EmptyState> : runtime.publicPlatform.history.slice().reverse().slice(0, 6).map((entry) => {
              const emphasis = PARTY_PLATFORM_ISSUES.slice()
                ...
                .join(" · ");
              return <EntityRow key={`${entry.date}:${entry.reason}`} title={entry.reason === "scenario_opening" ? "Opening platform" : entry.reason === "leadership_change" ? "Leadership platform" : "Annual party platform"} meta={`${entry.date}${entry.leaderId ? ` · Leader ${politicianDisplayName(props.catalog, entry.leaderId)}` : ""} · ${emphasis}`} />;
            })}
          </details>
      ...
      <SectionCard title="Nominations and leadership">
        ...
        {contests.map((c) => {
          ...
          const playerEndorsement = liveEndorsements.find(
            (endorsement) => endorsement.endorserType === "politician" && endorsement.endorserId === props.snap.playerPoliticianId,
          );
          ...
          return (
            <div key={c.id} className="contest-card">
              ...
              <StatusBadge tone={c.status === "open" ? "warn" : "idle"}>{c.status.replaceAll("_", " ")}</StatusBadge>
              <div className="muted">
                {Object.values(c.entries).filter((entry) => entry.status !== "potential").length} candidates
              </div>
              ...
              {c.winnerId ? <div>Winner: {politicianDisplayName(props.catalog, c.winnerId)}</div> : null}
              {playerEndorsement ? (
                <div className="player-endorsement-control">
                  <span>You endorsed <strong>{politicianDisplayName(props.catalog, playerEndorsement.targetId)}</strong> on {playerEndorsement.date}.</span>
                  <button type="button" className="btn danger quiet" onClick={() => run({ type: "WITHDRAW_ENDORSEMENT", endorsementId: playerEndorsement.id })}>Withdraw endorsement</button>
                </div>
              ) : null}
              {publicEndorsements.length ? (
                <details className="endorsement-network">
                  <summary>Public endorsement record ({liveEndorsements.length} current · {publicEndorsements.length - liveEndorsements.length} closed)</summary>
                  {publicEndorsements.slice(0, 20).map((endorsement) => {
                    ...
                    <EntityRow
                      key={endorsement.id}
                      title={endorsementActorName(endorsement.endorserType, endorsement.endorserId)}
...
function Terena(props: PageProps) {
  ...
  const electionChoices = [
    ...
    ...Object.values(props.snap.provincialRuntime.assemblyElections).map((election) => ({ id: election.id, date: election.date, type: "provincial_assembly", status: election.status, provinceId: election.provinceId as string | null })),
  ].sort((a, b) => {
  ...
  const tooltip = (selection: MapSelection) => {
    if (mode === "economy" && selection.kind === "province") {
      ...
      return <><strong>{selection.name}</strong><span> · {data ? data.summary : "No regional series"}</span></>;
    }
    if (mode === "campaign") {
    ...
    if (mode === "election") {
      ...
      if (selection.kind === "province" && regional) {
        ...
        if (race) return <><strong>{selection.name}</strong><span> · {race.winnerId ? `Winner ${politicianDisplayName(props.catalog, race.winnerId)}` : `${race.status.replace(/_/g, " ")} · ${Object.keys(race.candidates).length} candidates`}</span></>;
      }
      if (selection.kind === "province" && provincialAssembly) {
        ...
        if (race) {
          ...
          return <><strong>{selection.name}</strong><span> · {race.status === "resolved" ? `${tie ? "No single party plurality" : `${partyDisplayName(props.world, leading[0]?.[0] ?? null, props.snap)} leads`} · turnout ${race.turnoutRate == null ? "not recorded" : `${(race.turnoutRate * 100).toFixed(0)}%`}` : race.status.replace(/_/g, " ")}</span></>;
        }
      }
  ...
  const switchMapMode = (nextMode: MapMode) => {
    ...
    setSel((current) => {
      ...
      if (nextMode === "election") {
        ...
        if (activeMapElection?.type === "gubernatorial" || activeMapElection?.type === "provincial_assembly") return current.kind === "province" ? current : null;
        return null;
      }
      return current;
    });
  ...
  return (
    <div>
        ...
        <label className="map-election-picker">Election
          <select value={activeMapElection?.id ?? ""} onChange={(event) => {
            ...
            setSel((current) => {
              ...
              if (next.type === "gubernatorial" || next.type === "provincial_assembly") return current.kind === "province" ? current : null;
              return null;
            });
          }}>
      ...
      <MapDetailLayout
        ...
        detail={<div className="compact-map-inspector">
          ...
          {sel && place ? (
            <>
              ...
              {mode === "economy" && regionEcon ? (
                <div>{regionPublicEcon?.summary}</div>
              ) : null}
              {mode === "election" ? (
