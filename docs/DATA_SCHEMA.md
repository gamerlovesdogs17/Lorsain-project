...
- **Nomination:** vacancy → `awaiting_nomination` → `NOMINATE_CONSTITUTIONAL_JUDGE` (player President only; NPC President via Phase 2). Assembly confirmation next month. Player MP is never auto-voted; missed vote is `ABSTAIN`. NPC confirmation decisions weigh the nominee's public legal qualification and institutional credibility alongside party/relationship context so the 252-vote rule does not reduce appointments to party arithmetic.
...
### 14.10 Phase 11.3 institutional state (schema 13)

- **Migration:** save schema **13** adds institutional state through deterministic v12→v13 seeding. Existing player identity, date, offices, elections, laws and history remain intact. New chambers and legislators are seeded from the save's current canonical province/party state; migration does not invent backdated elections, bills, votes or careers.
...
- **Provincial Assemblies:** one chamber per province with 25–65 seats derived from canonical population, member IDs, party-seat totals, session label and next election. Elections store province, date/status, party vote/seat totals, elected member IDs and archive metadata. Votes are individual member choices. Bills store province, sponsor, subject, public title/summary, stage, vote/disposition IDs and bounded provincial effects. Governor sign/veto and two-thirds override use the same archived bill record.
- **Constitution:** `provincialRuntime.constitutionalRules` stores the supported live rules and their provenance; `constitutionalAmendments` stores sponsor, old/proposed value, 280-seat federal roll call, 18-month deadline, per-province ratification vote IDs, ratified/rejected provinces and final status. Province scheduling uses amendment-specific deterministic ordering, never lexicographic province order.
- **Party and caucus institutions:** party/faction leadership contests remain `PartyContest`s with explicit `selectorMethod`, scheduled close and immutable count archive. `legislatureRuntime.caucusLeadership` stores each represented party's floor leader, whip, selection/next-election dates and bounded priority bill IDs. `caucusContests` stores role, candidates, caucus ballots, result and history. Membership continues to be derived from live office and party state.
- **Federal legislation:** bills retain structured `PolicyItem`s but each public provision maps to one of 30 concrete categories and one of three named legal options. `optionId` is the stable policy-specific slug of the legal alternative (for example `national_protection`), never a generic ideological bucket. Schema-13 development saves containing the retired `low` / `current` / `high` aliases are accepted on read and canonicalized when a new policy item is created. An ordinary amendment targets a specific provision/category and records the replacement choice; version history and roll calls remain append-only public records.
- **Court:** nominations record qualification evidence. Cases may include federal–provincial disputes and challenges to provincial laws. Decisions store each participating justice's vote, majority/dissent author IDs, public majority/dissent text and resulting provincial-law disposition. The nine-seat bench remains derived from active office terms.
- **Organizations:** actor state stores policy alignment, affinity/trust relationship values, interactions, notes, endorsements and issue-specific bill pressure. Endorsements have an active lifecycle and may be withdrawn when the campaign ends or the public relationship materially breaks down.
- **Presentation state:** Calendar, global search, directory selection, map inspector and responsive layout state are ephemeral UI state and are not authoritative simulation fields.
