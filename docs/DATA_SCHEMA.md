...
- **Constitution:** `provincialRuntime.constitutionalRules` stores the supported live rules and their provenance; `constitutionalAmendments` stores sponsor, old/proposed value, political trigger/impetus, 280-seat federal roll call, optional proposal-specific deadline, per-province ratification vote IDs, ratified/rejected provinces and final status. Terena has no invented universal ratification deadline in v1. Province scheduling uses amendment-specific deterministic ordering, never lexicographic province order.
...
- **Federal legislation:** bills retain structured `PolicyItem`s but each public provision maps to one of 50 concrete categories and a variable list of two to five named legal options (161 alternatives total). `optionId` is the stable policy-specific slug of the legal alternative (for example `national_protection`), never a generic ideological bucket. Schema-13 development saves containing the retired `low` / `current` / `high` aliases are accepted on read and canonicalized when a new policy item is created. An ordinary amendment targets a specific provision/category and records the replacement choice; version history and roll calls remain append-only public records.
