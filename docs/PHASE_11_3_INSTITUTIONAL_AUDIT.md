...
Date: 2026-08-23; closeout re-review 2026-08-28

Interrupted-export baseline: `398d8e52b40c369f542dd2367fe38b86d173d719`

Actual resumed repository baseline: `2d215a606f090690b671ba3133357cb1a32fabb`
...
- The initially invented universal 18-month constitutional-ratification deadline had no canonical basis. Runtime and migration now use `null`; a deadline can exist only when an authorized future proposal-specific rule supplies it. Amendments still fail when all 21 Provincial Assemblies have voted without reaching 13.
- Schema 14 makes Phase 11.3's final persisted semantics explicit: personal provincial-election rankings, legislator service/election/sponsorship lists, bill cosponsors and party positions, caucus-contest politics, and amendment impetus. Direct v13→v14 regression coverage proves that the migration changes structure without inventing elections, office terms, or history.
- Browser population exposed duplicate endorsement feed rows and an “Unknown politician” label for provincial party organizations. Public actor presentation now resolves politicians, caucuses, provincial party organizations, and interest organizations, while the Campaign feed suppresses the duplicate received-side event.
- The map tooltip was technically correct but concatenated status fields in its accessible text. Explicit separators now make values such as `Darsen · planned · 0 candidates` unambiguous. The World map's five modes were also exercised with a keyboard-selected country and persistent public inspector.
- At 390 pixels, wide political tables could compress into unreadable columns even though the document itself did not overflow. Four-plus-column tables now retain a useful minimum width and scroll inside their own container; the page remains 390 pixels wide.
- The first fully completed 600-month seed exposed a lifecycle gap rather than a candidate-allocation bug: all 530 original national politicians were still marked active in 2078 and the mean active political age exceeded 92. Annual deterministic NPC retirement/death, filing-stage provincial retirement and one shared exit cleanup path now provide actual generational turnover. Assembly members and Governors remain protected from generic mid-term disappearance, while the active player is never retired or killed by AI.
- Turning the lifecycle on exposed filing attrition at month 460: a candidate who had affirmatively filed could retire in the January between filing opening and the election. Living filed candidates now finish the race, constituencies carry a two-candidate reserve, and the deadline performs one final pre-count recruitment check from existing provincial careers. Election resolution still cannot generate a candidate.
- Larger 2030 fields exposed a restore-only committee reconciliation. The incoming Assembly was seated on assumption, but its committees could remain attached to the outgoing membership until the next monthly legislature pass; restoring immediately repaired them and changed the hash. Committee reseating now occurs atomically at assumption and at the end of any turn containing a scheduled office transition.
