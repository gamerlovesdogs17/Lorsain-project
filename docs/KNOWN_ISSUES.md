...
| P96-NEWS-PROSE | NONBLOCKING | News groups outlet treatments under public events; it does not write original newspaper copy. Phase 11.3 still uses modest fact-injection + framing templates (see P113-NEWS-HISTORY-PROSE). |
...
**Phase 11.4 presentation/content completion is NOT YET ACCEPTED** — Priority 0–1 landed (truthfulness, fixtures, unified Constitution document inspector with red/green diffs, Assembly chamber+leadership, Campaign/Election Night evidence, repetition audit). Priority 2 depth and headline uniqueness remain incomplete; see `docs/PHASE_11_4_RESULTS.md`. Phase 11.5 has not begun.
...
| P113-GENERATIONAL-TURNOVER | **FIXED** | The first completed 600-month audit showed 530 of 530 original politicians still active with a mean active age above 92. Annual deterministic NPC retirement/death, filing-stage provincial retirement and shared vacancy/candidacy cleanup now renew the class without consuming gameplay RNG or choosing the player's exit. |
| P113-FILING-ATTRITION | **FIXED** | Filed living candidates cannot silently retire during their race. Assembly fields open with a two-person reserve and perform a final lawful recruitment check at the filing deadline; count-time candidate generation remains prohibited. |
| P113-COMMITTEE-RESTORE | **FIXED** | A newly assumed Assembly now reseats committees in the same transition. Scheduled office changes receive end-of-turn institutional reconciliation, so loading a save no longer changes committee membership or the simulation hash. |
...
| P113-LAW-COPY | **FIXED** | Bills use 50 concrete provision categories / 161 policy-specific options, targeted amendments and deterministic editorial titles instead of IDs or “moderate on …” descriptions. A minority of provisions remain scalar-adjacent (duration, eligibility bands, timing/appropriation size) while still offering genuine institutional designs rather than a disguised less/same/more helper. |
...
| P113-RELEASE-ENG | DEFERRED TO 11.5 | Route/data splitting, immutable-save compaction, visual-diff CI, 100×600 soak. |
